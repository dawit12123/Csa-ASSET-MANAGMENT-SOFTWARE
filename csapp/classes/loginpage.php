...
class LoginPage extends RunnerPage
{
	...
	protected function sendCodeBySMS()
	{
		...
		else
		{
			...
			$this->message = str_replace( "%phone%", $this->getMaskedPhone(), mlang_message("MESSAGE_SENT_TO_PNONE") );
		}
	...
	protected function reportSecondAuthStepResult( $verified )
	{
		...
		if( !$verified )
			$returnJSON["message"] = mlang_message("WRONG_CODE");
		else
		{
			if( $this->mode == LOGIN_POPUP )
				$returnJSON["message"] = mlang_message("SUCCES_LOGGED_IN");
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("csa_users")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$strSQL = "select * from ".$this->connection->addTableWrappers( "csa_users" )
			." where "
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "csa_users", $data, $blobfields, $this );
			if( !$retval )
				return false;
