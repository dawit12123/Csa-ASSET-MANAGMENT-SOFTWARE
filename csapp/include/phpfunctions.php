...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="eaempassets" && $field=="asset_id")
	{
		$sql = "select * from asset where asset_id='".$data['asset_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['asset_name'];;
		return $value;
	}
				if($table=="eaempassets" && $field=="employee_id")
	{
		$sql="select * from employee where id='".$data['employee_id']."'";
$r=CustomQuery($sql);
$d=db_fetch_array($r);
...
$value=$d['firstname'].' '.$d['lastname'];;
		return $value;
				...
				if($table=="model19fromemployee" && $field=="asset_id")
	{
		$sql="select * from asset where asset_id='".$data['asset_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['asset_name'];;
		return $value;
				...
				if($table=="model19fromemployee" && $field=="totalprice")
	{
		$value=$data['quantity'] * $data['unitprice'];;
		return $value;
	}
				if($table=="model19fromemployee" && $field=="donor_id")
	{
		$sql = "select * from employee where id='".$data['donor_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['firstname'].' '.$data1['lastname'];;
		return $value;
	}
				if($table=="model19frompurchase" && $field=="receiver_id")
	{
		$userData = Security::currentUserData();
$value=$userData[$id];;
		return $value;
	}
				if($table=="model20" && $field=="asset_id")
	{
		$sql="select * from asset where asset_id='".$data['asset_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['asset_name'];;
		return $value;
				...
				if($table=="model20" && $field=="manager_id")
	{
		$sql = "select * from employee where id='".$data['manager_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['firstname'].' '.$data1['lastname'];;
		return $value;
	}
				if($table=="model20" && $field=="empid")
	{
		
$sql="select * from employee where id='".$data['empid']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['firstname'].' '.$data1['lastname'];;
		return $value;
	}
				if($table=="model20" && $field=="department")
	{
		$sql="select * from departments where department_id='".$data['department']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['department_name'];

;
		return $value;
	}
				if($table=="request_info" && $field=="employee_id")
	{
		$sql = "select * from employee where id='".$data['employee_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['firstname'].' '.$data1['lastname'];;
		return $value;
				...
				if($table=="request_info" && $field=="asset_id")
	{
		$sql = "select * from asset where asset_id='".$data['asset_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['asset_name'];;
		return $value;
				...
				if($table=="request_info" && $field=="department")
	{
		$sql="select * from departments where department_id='".$data['department']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['department_name'];;
		return $value;
				...
				if($table=="employee" && $field=="job_id")
	{
		$sql = "select * from jobs where job_id='".$data['job_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['job_title'];;
		return $value;
	}
				if($table=="employee" && $field=="manager_id")
	{
		$sql = "select * from employee where id='".$data['manager_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['firstname'].' '.$data1['lastname'];;
		return $value;
				...
				if($table=="employee" && $field=="workunit_id")
	{
		$sql = "select * from workunit where workunit_id='".$data['workunit_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['workunit_name'];;
		return $value;
	}
				if($table=="mode22" && $field=="asset_id")
	{
		$sql = "select * from asset where asset_id='".$data['asset_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['asset_name'];;
		return $value;
				...
				if($table=="mode22" && $field=="receiver_id")
	{
		$sql = "select * from employee where id='".$data['receiver_id']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['firstname'].' '.$data1['lastname'];;
		return $value;
	}
				if($table=="checkapproval" && $field=="assetid")
	{
		$sql = "select * from asset where asset_id='".$data['assetid']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
$value=$data1['asset_name'];;
		return $value;
	}
				if($table=="checkapproval" && $field=="empid")
	{
		$sql = "select * from employee where id='".$data['empid']."'";
$rs = CustomQuery($sql);
$data1 = db_fetch_array($rs);
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="asset" && $field=="registration_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="asset" && $field=="taken")
	{
		return no;
	}
				if($table=="csa_users" && $field=="active")
	{
		return enabled;
	}
				if($table=="eaempassets" && $field=="issued_date")
	{
		return strftime("%Y-%m-%d");
	}
				if($table=="model19fromemployee" && $field=="totalprice")
	{
		return $srchObj->getFieldValue("unitprice")*$pageObj->getFieldValue("quantity");;
	}
				if($table=="model19fromemployee" && $field=="received_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="model19fromemployee" && $field=="receiver_id")
	{
		return Security::getUserData($id);;
	}
				if($table=="model19frompurchase" && $field=="received_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="model20" && $field=="empid")
	{
		return $userData = Security::currentUserData();$value=$userData['id'];;
	}
				if($table=="model20" && $field=="date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="request_info" && $field=="seen")
	{
		return no;
	}
				if($table=="request_info" && $field=="taked")
	{
		return no;
	}
				if($table=="mode22" && $field=="issued_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="asset in store" && $field=="registration_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="asset in store" && $field=="taken")
	{
		return no;
	}
	return "";
